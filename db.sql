-------BASE DE DATOS---------

CREATE DATABASE FIDE_FDB_ESTUDIO_CASO_DOS
GO

-----USAR BASE DE DATOS-------

USE FIDE_FDB_ESTUDIO_CASO_DOS;
GO

---------TABLEAS----------

CREATE TABLE FIDE_ESTADO_TB (
    ID_ESTADO INT,
    ESTADO VARCHAR(50) NOT NULL,
    CONSTRAINT FIDE_ESTADO_ID_PK PRIMARY KEY (ID_ESTADO)
);
GO

CREATE TABLE FIDE_ESPECIALIDAD_TB (
    ID_ESPECIALIDAD INT IDENTITY(1,1),
    NOMBRE_ESPECIALIDAD VARCHAR(100) NOT NULL,
    ID_ESTADO INT,
    CONSTRAINT FIDE_ESPECIALIDAD_ID_PK PRIMARY KEY (ID_ESPECIALIDAD),
    CONSTRAINT FIDE_ESPECIALIDAD_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE FIDE_ALUMNO_TB (
    ID_ALUMNO VARCHAR(10),
    NOMBRE_ALUMNO VARCHAR(100) NOT NULL,
    PRIMER_APELLIDO VARCHAR(100) NOT NULL,
    SEGUNDO_APELLIDO VARCHAR(100) NOT NULL,
    ID_ESPECIALIDAD INT,
    ID_ESTADO INT,
    CONSTRAINT FIDE_ALUMNO_ID_PK PRIMARY KEY (ID_ALUMNO),
    CONSTRAINT FIDE_ALUMNO_ESPECIALIDAD_FK FOREIGN KEY (ID_ESPECIALIDAD) REFERENCES FIDE_ESPECIALIDAD_TB(ID_ESPECIALIDAD),
    CONSTRAINT FIDE_ALUMNO_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE FIDE_OFICINA_TB (
    ID_OFICINA INT IDENTITY(1,1),
    OFICINA VARCHAR(20) NOT NULL,
    ID_ESTADO INT,
    CONSTRAINT FIDE_OFICINA_ID_PK PRIMARY KEY (ID_OFICINA),
    CONSTRAINT FIDE_OFICINA_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE FIDE_DOCENTE_TB (
    ID_DOCENTE INT IDENTITY(1,1),
    NOMBRE_DOCENTE VARCHAR(100) NOT NULL,
    ID_OFICINA INT,
    ID_ESTADO INT,
    CONSTRAINT FIDE_DOCENTE_ID_PK PRIMARY KEY (ID_DOCENTE),
    CONSTRAINT FIDE_DOCENTE_OFICINA_FK FOREIGN KEY (ID_OFICINA) REFERENCES FIDE_OFICINA_TB(ID_OFICINA),
    CONSTRAINT FIDE_DOCENTE_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE FIDE_CURSO_TB (
    ID_CURSO VARCHAR(10),
    NOMBRE_CURSO VARCHAR(100) NOT NULL,
    CURSO VARCHAR(5),
    ID_ESTADO INT,
    CONSTRAINT FIDE_CURSO_ID_PK PRIMARY KEY (ID_CURSO),
    CONSTRAINT FIDE_CURSO_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE CURSO_ALUMNOS_TB (
    ID_CURSO VARCHAR(10),
    ID_ALUMNO VARCHAR(10),
    ID_ESTADO INT,
    CONSTRAINT CURSO_ALUMNOS_PK PRIMARY KEY (ID_CURSO, ID_ALUMNO),
    CONSTRAINT CURSO_ALUMNOS_CURSO_FK FOREIGN KEY (ID_CURSO) REFERENCES FIDE_CURSO_TB(ID_CURSO),
    CONSTRAINT CURSO_ALUMNOS_ALUMNO_FK FOREIGN KEY (ID_ALUMNO) REFERENCES FIDE_ALUMNO_TB(ID_ALUMNO),
    CONSTRAINT CURSO_ALUMNOS_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE FIDE_CURSO_IMPARTIDO_TB (
    ID_CURSO VARCHAR(10),
    ID_DOCENTE INT,
    ID_ESTADO INT,
    CONSTRAINT FIDE_CURSO_IMPARTIDO_PK PRIMARY KEY (ID_CURSO, ID_DOCENTE),
    CONSTRAINT FIDE_CURSO_IMPARTIDO_CURSO_FK FOREIGN KEY (ID_CURSO) REFERENCES FIDE_CURSO_TB(ID_CURSO),
    CONSTRAINT FIDE_CURSO_IMPARTIDO_DOCENTE_FK FOREIGN KEY (ID_DOCENTE) REFERENCES FIDE_DOCENTE_TB(ID_DOCENTE),
    CONSTRAINT FIDE_CURSO_IMPARTIDO_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE FIDE_ACCION_AUDITORIA_TB (
    ID_ACCION_AUDITORIA INT,
    ACCION VARCHAR(50) NOT NULL,
    ID_ESTADO INT,
    CONSTRAINT FIDE_ACCION_AUDITORIA_ID_PK PRIMARY KEY (ID_ACCION_AUDITORIA),
    CONSTRAINT FIDE_ACCION_AUDITORIA_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

CREATE TABLE FIDE_AUDITORIA_TB (
    ID_AUDITORIA INT IDENTITY(1,1),
    ID_ACCION_AUDITORIA INT,
    TABLA_AFECTADA VARCHAR(100) NOT NULL,
    FECHA_HORA DATETIME NOT NULL,
    ID_DOCENTE INT,
    ID_ESTADO INT,
    CONSTRAINT FIDE_AUDITORIA_ID_PK PRIMARY KEY (ID_AUDITORIA),
    CONSTRAINT FIDE_AUDITORIA_ACCION_FK FOREIGN KEY (ID_ACCION_AUDITORIA) REFERENCES FIDE_ACCION_AUDITORIA_TB(ID_ACCION_AUDITORIA),
    CONSTRAINT FIDE_AUDITORIA_DOCENTE_FK FOREIGN KEY (ID_DOCENTE) REFERENCES FIDE_DOCENTE_TB(ID_DOCENTE),
    CONSTRAINT FIDE_AUDITORIA_ESTADO_FK FOREIGN KEY (ID_ESTADO) REFERENCES FIDE_ESTADO_TB(ID_ESTADO)
);
GO

-----------------FIDE_ESTADO_TB---------------------

SELECT * FROM FIDE_ESTADO_TB;
GO 

-------- INSERTAR ESTADO --------

CREATE PROCEDURE FIDE_ESTADO_INSERTAR_SP
    @ID_ESTADO INT,
    @ESTADO VARCHAR(50)
AS
BEGIN
    IF EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID DEL ESTADO YA EXISTE.', 16,1);
        RETURN;
    END;

    INSERT INTO FIDE_ESTADO_TB (ID_ESTADO, ESTADO)
    VALUES (@ID_ESTADO, @ESTADO);
END;
GO

EXEC FIDE_ESTADO_INSERTAR_SP 1, 'ACTIVO';
GO
EXEC FIDE_ESTADO_INSERTAR_SP 2, 'INACTIVO';
GO
EXEC FIDE_ESTADO_INSERTAR_SP 3, 'PENDIENTE';
GO
EXEC FIDE_ESTADO_INSERTAR_SP 4, 'SUSPENDIDO';
GO
EXEC FIDE_ESTADO_INSERTAR_SP 5, 'ELIMINADO';
GO


-----------------FIDE_ESPECIALIDAD_TB---------------------

SELECT * FROM FIDE_ESPECIALIDAD_TB;
GO 

-------- INSERTAR ESPECIALIDAD --------

CREATE PROCEDURE FIDE_ESPECIALIDAD_INSERTAR_SP
    @NOMBRE_ESPECIALIDAD VARCHAR(100),
    @ID_ESTADO INT
AS
BEGIN
    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO FIDE_ESPECIALIDAD_TB (NOMBRE_ESPECIALIDAD, ID_ESTADO)
    VALUES (@NOMBRE_ESPECIALIDAD, @ID_ESTADO);
END;
GO

EXEC FIDE_ESPECIALIDAD_INSERTAR_SP 'Ingeniería de Software', 1;
GO
EXEC FIDE_ESPECIALIDAD_INSERTAR_SP 'Ciberseguridad', 1;
GO
EXEC FIDE_ESPECIALIDAD_INSERTAR_SP 'Bases de Datos', 2;
GO
EXEC FIDE_ESPECIALIDAD_INSERTAR_SP 'Matemática Aplicada', 1;
GO
EXEC FIDE_ESPECIALIDAD_INSERTAR_SP 'Redes y Telecomunicaciones', 3;
GO


-----------------FIDE_ALUMNO_TB---------------------

SELECT * FROM FIDE_ALUMNO_TB;
GO 

-------- INSERTAR ALUMNO --------

CREATE PROCEDURE FIDE_ALUMNO_INSERTAR_SP
    @ID_ALUMNO VARCHAR(10),
    @NOMBRE_ALUMNO VARCHAR(100),
    @PRIMER_APELLIDO VARCHAR(100),
    @SEGUNDO_APELLIDO VARCHAR(100),
    @ID_ESTADO INT
AS
BEGIN
    IF EXISTS (SELECT 1 FROM FIDE_ALUMNO_TB WHERE ID_ALUMNO = @ID_ALUMNO)
    BEGIN
        RAISERROR('EL ID_ALUMNO YA EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO FIDE_ALUMNO_TB (ID_ALUMNO, NOMBRE_ALUMNO, PRIMER_APELLIDO, SEGUNDO_APELLIDO,ID_ESTADO)
    VALUES (@ID_ALUMNO, @NOMBRE_ALUMNO, @PRIMER_APELLIDO, @SEGUNDO_APELLIDO, @ID_ESTADO);
END;
GO

EXEC FIDE_ALUMNO_INSERTAR_SP 'A001', 'Juan', 'Pérez', 'Ramírez', 1;
GO
EXEC FIDE_ALUMNO_INSERTAR_SP 'A002', 'María', 'López', 'García', 1;
GO
EXEC FIDE_ALUMNO_INSERTAR_SP 'A003', 'Carlos', 'Jiménez', 'Vargas', 2;
GO
EXEC FIDE_ALUMNO_INSERTAR_SP 'A004', 'Ana', 'Torres', 'Rojas', 1;
GO
EXEC FIDE_ALUMNO_INSERTAR_SP 'A005', 'Luis', 'Castro', 'Hernández', 3;
GO

DROP PROCEDURE FIDE_ALUMNO_INSERTAR_SP


-----------------FIDE_OFICINA_TB---------------------

SELECT * FROM FIDE_OFICINA_TB;
GO 

-------- INSERTAR OFICINA --------

CREATE PROCEDURE FIDE_OFICINA_INSERTAR_SP
    @OFICINA VARCHAR(20),
    @ID_ESTADO INT
AS
BEGIN
    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO FIDE_OFICINA_TB (OFICINA, ID_ESTADO)
    VALUES (@OFICINA, @ID_ESTADO);
END;
GO

EXEC FIDE_OFICINA_INSERTAR_SP 'Oficina Central', 1;
GO
EXEC FIDE_OFICINA_INSERTAR_SP 'Aulas Norte', 1;
GO
EXEC FIDE_OFICINA_INSERTAR_SP 'Edificio C', 2;
GO
EXEC FIDE_OFICINA_INSERTAR_SP 'Laboratorios', 1;
GO
EXEC FIDE_OFICINA_INSERTAR_SP 'Extensión Sur', 3;
GO


-----------------FIDE_DOCENTE_TB---------------------

SELECT * FROM FIDE_DOCENTE_TB;
GO 

-------- INSERTAR DOCENTE --------

CREATE PROCEDURE FIDE_DOCENTE_INSERTAR_SP
    @NOMBRE_DOCENTE VARCHAR(100),
    @ID_OFICINA INT,
    @ID_ESTADO INT
AS
BEGIN
    IF NOT EXISTS (SELECT 1 FROM FIDE_OFICINA_TB WHERE ID_OFICINA = @ID_OFICINA)
    BEGIN
        RAISERROR('EL ID_OFICINA NO EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO FIDE_DOCENTE_TB (NOMBRE_DOCENTE, ID_OFICINA, ID_ESTADO)
    VALUES (@NOMBRE_DOCENTE, @ID_OFICINA, @ID_ESTADO);
END;
GO

EXEC FIDE_DOCENTE_INSERTAR_SP 'Andrea Rojas', 1, 1;
GO
EXEC FIDE_DOCENTE_INSERTAR_SP 'Fernando Gómez', 2, 1;
GO
EXEC FIDE_DOCENTE_INSERTAR_SP 'Sofía Mora', 3, 2;
GO
EXEC FIDE_DOCENTE_INSERTAR_SP 'Daniel Salas', 4, 1;
GO
EXEC FIDE_DOCENTE_INSERTAR_SP 'Laura Vega', 5, 3;
GO


-----------------FIDE_CURSO_TB---------------------

SELECT * FROM FIDE_CURSO_TB;
GO 

-------- INSERTAR CURSO --------

CREATE PROCEDURE FIDE_CURSO_INSERTAR_SP
    @ID_CURSO VARCHAR(10),
    @NOMBRE_CURSO VARCHAR(100),
    @CURSO VARCHAR(5),
    @ID_ESTADO INT
AS
BEGIN
    IF EXISTS (SELECT 1 FROM FIDE_CURSO_TB WHERE ID_CURSO = @ID_CURSO)
    BEGIN
        RAISERROR('EL ID_CURSO YA EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO FIDE_CURSO_TB (ID_CURSO, NOMBRE_CURSO, CURSO, ID_ESTADO)
    VALUES (@ID_CURSO, @NOMBRE_CURSO, @CURSO, @ID_ESTADO);
END;
GO

EXEC FIDE_CURSO_INSERTAR_SP 'C001', 'Programación I', '101', 1;
GO
EXEC FIDE_CURSO_INSERTAR_SP 'C002', 'Redes Básicas', '102', 1;
GO
EXEC FIDE_CURSO_INSERTAR_SP 'C003', 'Criptografía', '103', 2;
GO
EXEC FIDE_CURSO_INSERTAR_SP 'C004', 'Cálculo II', '104', 1;
GO
EXEC FIDE_CURSO_INSERTAR_SP 'C005', 'SQL Avanzado', '105', 3;
GO


-----------------CURSO_ALUMNOS_TB---------------------

SELECT * FROM CURSO_ALUMNOS_TB;
GO 

-------- INSERTAR CURSO ALUMNO --------

CREATE PROCEDURE CURSO_ALUMNOS_INSERTAR_SP
    @ID_CURSO VARCHAR(10),
    @ID_ALUMNO VARCHAR(10),
    @ID_ESTADO INT
AS
BEGIN
    IF EXISTS (SELECT 1 FROM CURSO_ALUMNOS_TB WHERE ID_CURSO = @ID_CURSO AND ID_ALUMNO = @ID_ALUMNO)
    BEGIN
        RAISERROR('LA RELACIÓN CURSO-ALUMNO YA EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_CURSO_TB WHERE ID_CURSO = @ID_CURSO)
    BEGIN
        RAISERROR('EL ID_CURSO NO EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_ALUMNO_TB WHERE ID_ALUMNO = @ID_ALUMNO)
    BEGIN
        RAISERROR('EL ID_ALUMNO NO EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO CURSO_ALUMNOS_TB (ID_CURSO, ID_ALUMNO, ID_ESTADO)
    VALUES (@ID_CURSO, @ID_ALUMNO, @ID_ESTADO);
END;
GO

EXEC CURSO_ALUMNOS_INSERTAR_SP 'C001', 'A001', 1;
GO
EXEC CURSO_ALUMNOS_INSERTAR_SP 'C002', 'A002', 1;
GO
EXEC CURSO_ALUMNOS_INSERTAR_SP 'C003', 'A003', 2;
GO
EXEC CURSO_ALUMNOS_INSERTAR_SP 'C004', 'A004', 1;
GO
EXEC CURSO_ALUMNOS_INSERTAR_SP 'C005', 'A005', 3;
GO


-----------------FIDE_CURSO_IMPARTIDO_TB---------------------

SELECT * FROM FIDE_CURSO_IMPARTIDO_TB;
GO 

-------- INSERTAR CURSO IMPARTIDO --------

CREATE PROCEDURE FIDE_CURSO_IMPARTIDO_INSERTAR_SP
    @ID_CURSO VARCHAR(10),
    @ID_DOCENTE INT,
    @ID_ESTADO INT
AS
BEGIN
    -- Validar existencia de curso
    IF NOT EXISTS (SELECT 1 FROM FIDE_CURSO_TB WHERE ID_CURSO = @ID_CURSO)
    BEGIN
        RAISERROR('EL ID_CURSO NO EXISTE.', 16, 1);
        RETURN;
    END;

    -- Validar existencia de docente
    IF NOT EXISTS (SELECT 1 FROM FIDE_DOCENTE_TB WHERE ID_DOCENTE = @ID_DOCENTE)
    BEGIN
        RAISERROR('EL ID_DOCENTE NO EXISTE.', 16, 1);
        RETURN;
    END;

    -- Validar existencia de estado
    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    -- Validar duplicados
    IF EXISTS (
        SELECT 1 FROM FIDE_CURSO_IMPARTIDO_TB
        WHERE ID_CURSO = @ID_CURSO AND ID_DOCENTE = @ID_DOCENTE
    )
    BEGIN
        RAISERROR('YA EXISTE LA RELACIÓN CURSO-DOCENTE.', 16, 1);
        RETURN;
    END;

    -- Insertar
    INSERT INTO FIDE_CURSO_IMPARTIDO_TB (ID_CURSO, ID_DOCENTE, ID_ESTADO)
    VALUES (@ID_CURSO, @ID_DOCENTE, @ID_ESTADO);
END;
GO

EXEC FIDE_CURSO_IMPARTIDO_INSERTAR_SP 'C001', 1, 1;
GO
EXEC FIDE_CURSO_IMPARTIDO_INSERTAR_SP 'C002', 2, 1;
GO
EXEC FIDE_CURSO_IMPARTIDO_INSERTAR_SP 'C003', 3, 2;
GO
EXEC FIDE_CURSO_IMPARTIDO_INSERTAR_SP 'C004', 4, 1;
GO
EXEC FIDE_CURSO_IMPARTIDO_INSERTAR_SP 'C005', 5, 3;
GO

-----------------FIDE_ACCION_AUDITORIA_TB---------------------

SELECT * FROM FIDE_ACCION_AUDITORIA_TB;
GO 

-------- INSERTAR ACCION AUDITORIA --------

CREATE PROCEDURE FIDE_ACCION_AUDITORIA_INSERTAR_SP
    @ID_ACCION_AUDITORIA INT,
    @ACCION VARCHAR(50),
    @ID_ESTADO INT
AS
BEGIN
    IF EXISTS (SELECT 1 FROM FIDE_ACCION_AUDITORIA_TB WHERE ID_ACCION_AUDITORIA = @ID_ACCION_AUDITORIA)
    BEGIN
        RAISERROR('EL ID_ACCION_AUDITORIA YA EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO FIDE_ACCION_AUDITORIA_TB (ID_ACCION_AUDITORIA, ACCION, ID_ESTADO)
    VALUES (@ID_ACCION_AUDITORIA, @ACCION, @ID_ESTADO);
END;
GO

EXEC FIDE_ACCION_AUDITORIA_INSERTAR_SP 1, 'INSERT', 1;
GO
EXEC FIDE_ACCION_AUDITORIA_INSERTAR_SP 2, 'UPDATE', 1;
GO
EXEC FIDE_ACCION_AUDITORIA_INSERTAR_SP 3, 'DELETE', 1;
GO
EXEC FIDE_ACCION_AUDITORIA_INSERTAR_SP 4, 'LOGIN', 1;
GO
EXEC FIDE_ACCION_AUDITORIA_INSERTAR_SP 5, 'LOGOUT', 1;
GO


-----------------FIDE_AUDITORIA_TB---------------------

SELECT * FROM FIDE_AUDITORIA_TB;
GO 

-------- INSERTAR AUDITORIA --------

CREATE PROCEDURE FIDE_AUDITORIA_INSERTAR_SP
    @ID_ACCION_AUDITORIA INT,
    @TABLA_AFECTADA VARCHAR(100),
    @FECHA_HORA DATETIME,
    @ID_DOCENTE INT,
    @ID_ESTADO INT
AS
BEGIN
    IF NOT EXISTS (SELECT 1 FROM FIDE_ACCION_AUDITORIA_TB WHERE ID_ACCION_AUDITORIA = @ID_ACCION_AUDITORIA)
    BEGIN
        RAISERROR('EL ID_ACCION_AUDITORIA NO EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_DOCENTE_TB WHERE ID_DOCENTE = @ID_DOCENTE)
    BEGIN
        RAISERROR('EL ID_DOCENTE NO EXISTE.', 16, 1);
        RETURN;
    END;

    IF NOT EXISTS (SELECT 1 FROM FIDE_ESTADO_TB WHERE ID_ESTADO = @ID_ESTADO)
    BEGIN
        RAISERROR('EL ID_ESTADO NO EXISTE.', 16, 1);
        RETURN;
    END;

    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (@ID_ACCION_AUDITORIA, @TABLA_AFECTADA, @FECHA_HORA, @ID_DOCENTE, @ID_ESTADO);
END;
GO

DECLARE @FECHA DATETIME = GETDATE();
    
EXEC FIDE_AUDITORIA_INSERTAR_SP 1, 'FIDE_CURSO_TB', @FECHA, 1, 1;
EXEC FIDE_AUDITORIA_INSERTAR_SP 2, 'FIDE_ALUMNO_TB', @FECHA, 2, 1;
EXEC FIDE_AUDITORIA_INSERTAR_SP 3, 'FIDE_ESPECIALIDAD_TB', @FECHA, 3, 2;
EXEC FIDE_AUDITORIA_INSERTAR_SP 4, 'FIDE_DOCENTE_TB', @FECHA, 4, 1;
EXEC FIDE_AUDITORIA_INSERTAR_SP 5, 'FIDE_CURSO_IMPARTIDO_TB', @FECHA, 5, 3;
GO

----------FUNCION NOMBRE COMBLETO------------
CREATE FUNCTION FIDE_ALUMNO_TB_OBTENER_NOMBRE_COMPLETO_FN (@ID_ALUMNO VARCHAR(10))
RETURNS NVARCHAR(100)
AS
BEGIN
    DECLARE @NOMBRE_ALUMNO NVARCHAR(100);
    SELECT @NOMBRE_ALUMNO = NOMBRE_ALUMNO + ' ' + PRIMER_APELLIDO + ' ' + SEGUNDO_APELLIDO
    FROM FIDE_ALUMNO_TB
    WHERE ID_ALUMNO = @ID_ALUMNO;
    RETURN @NOMBRE_ALUMNO;
END;
GO

SELECT DBO.FIDE_ALUMNO_TB_OBTENER_NOMBRE_COMPLETO_FN('A001') AS 'NOMBRE COMPLETO DEL USUARIO SEGUN ID'
GO

----------FUNCION CANTIDAD DE ALUMNOS DE UN CURSO------------
CREATE FUNCTION FIDE_CURSO_TB_CANTIDAD_ALUMNOS_CURSO_FN(@ID_CURSO VARCHAR(10))
RETURNS INT
AS
BEGIN
    DECLARE @TOTAL INT;
    SELECT @TOTAL = COUNT(*)
    FROM CURSO_ALUMNOS_TB
    WHERE ID_CURSO = @ID_CURSO;
    RETURN @TOTAL;
END;
GO

SELECT DBO.FIDE_CURSO_TB_CANTIDAD_ALUMNOS_CURSO_FN('C001') AS 'CANTIDAD DE ALUMNOS EN UN CURSO'
GO

----------FUNCION ESPECIALIDAD ALUMNO------------
CREATE FUNCTION FIDE_ALUMNO_TB_OBTENER_ESPECIALIDAD_FN (@ID_ALUMNO VARCHAR(10))
RETURNS NVARCHAR(100)
AS
BEGIN
    DECLARE @ESPECIALIDAD_ALUMNO NVARCHAR(100);
    SELECT @ESPECIALIDAD_ALUMNO = E.NOMBRE_ESPECIALIDAD
    FROM FIDE_ALUMNO_TB A
    INNER JOIN FIDE_ESPECIALIDAD_TB E ON A.ID_ESPECIALIDAD = E.ID_ESPECIALIDAD
    WHERE A.ID_ALUMNO = @ID_ALUMNO;
    RETURN @ESPECIALIDAD_ALUMNO;
END;
GO

SELECT DBO.FIDE_ALUMNO_TB_OBTENER_ESPECIALIDAD_FN('A001') AS 'ESPECIALIDAD DEL ALUMNO SEGUN ID'
GO

-----TOTAL DE CURSOS IMPARTIDOS POR EL DOCENTE'
CREATE FUNCTION FIDE_CURSO_IMPARTIDO_TB_CANTIDAD_CURSOS_DOCENTE_FN(@ID_DOCENTE INT)
RETURNS INT
AS
BEGIN
    DECLARE @TOTAL INT;
    SELECT @TOTAL = COUNT(*)
    FROM FIDE_CURSO_IMPARTIDO_TB
    WHERE ID_DOCENTE = @ID_DOCENTE;
    RETURN @TOTAL;
END;
GO

SELECT DBO.FIDE_CURSO_IMPARTIDO_TB_CANTIDAD_CURSOS_DOCENTE_FN(5) AS 'TOTAL DE CURSOS IMPARTIDOS POR EL DOCENTE'
GO

-----------------TRIGGERS DE AUDITORIA---------------------

-------- TRIGGERS FIDE_ESPECIALIDAD_TB --------
CREATE TRIGGER FIDE_ESPECIALIDAD_TB_INSERT_TR
ON FIDE_ESPECIALIDAD_TB
AFTER INSERT
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (1, 'FIDE_ESPECIALIDAD_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_ESPECIALIDAD_TB_UPDATE_TR
ON FIDE_ESPECIALIDAD_TB
AFTER UPDATE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (2, 'FIDE_ESPECIALIDAD_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_ESPECIALIDAD_TB_DELETE_TR
ON FIDE_ESPECIALIDAD_TB
AFTER DELETE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (3, 'FIDE_ESPECIALIDAD_TB', GETDATE(), 1, 1);
END;
GO

-------- TRIGGERS FIDE_ALUMNO_TB --------
CREATE TRIGGER FIDE_ALUMNO_TB_INSERT_TR
ON FIDE_ALUMNO_TB
AFTER INSERT
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (1, 'FIDE_ALUMNO_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_ALUMNO_TB_UPDATE_TR
ON FIDE_ALUMNO_TB
AFTER UPDATE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (2, 'FIDE_ALUMNO_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_ALUMNO_TB_DELETE_TR
ON FIDE_ALUMNO_TB
AFTER DELETE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (3, 'FIDE_ALUMNO_TB', GETDATE(), 1, 1);
END;
GO

-------- TRIGGERS FIDE_OFICINA_TB --------
CREATE TRIGGER FIDE_OFICINA_TB_INSERT_TR
ON FIDE_OFICINA_TB
AFTER INSERT
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (1, 'FIDE_OFICINA_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_OFICINA_TB_UPDATE_TR
ON FIDE_OFICINA_TB
AFTER UPDATE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (2, 'FIDE_OFICINA_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_OFICINA_TB_DELETE_TR
ON FIDE_OFICINA_TB
AFTER DELETE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (3, 'FIDE_OFICINA_TB', GETDATE(), 1, 1);
END;
GO

-------- TRIGGERS FIDE_DOCENTE_TB --------
CREATE TRIGGER FIDE_DOCENTE_TB_INSERT_TR
ON FIDE_DOCENTE_TB
AFTER INSERT
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (1, 'FIDE_DOCENTE_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_DOCENTE_TB_UPDATE_TR
ON FIDE_DOCENTE_TB
AFTER UPDATE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (2, 'FIDE_DOCENTE_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_DOCENTE_TB_DELETE_TR
ON FIDE_DOCENTE_TB
AFTER DELETE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (3, 'FIDE_DOCENTE_TB', GETDATE(), 1, 1);
END;
GO

-------- TRIGGERS FIDE_CURSO_TB --------
CREATE TRIGGER FIDE_CURSO_TB_INSERT_TR
ON FIDE_CURSO_TB
AFTER INSERT
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (1, 'FIDE_CURSO_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_CURSO_TB_UPDATE_TR
ON FIDE_CURSO_TB
AFTER UPDATE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (2, 'FIDE_CURSO_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_CURSO_TB_DELETE_TR
ON FIDE_CURSO_TB
AFTER DELETE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (3, 'FIDE_CURSO_TB', GETDATE(), 1, 1);
END;
GO

-------- TRIGGERS CURSO_ALUMNOS_TB --------
CREATE TRIGGER CURSO_ALUMNOS_TB_INSERT_TR
ON CURSO_ALUMNOS_TB
AFTER INSERT
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (1, 'CURSO_ALUMNOS_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER CURSO_ALUMNOS_TB_UPDATE_TR
ON CURSO_ALUMNOS_TB
AFTER UPDATE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (2, 'CURSO_ALUMNOS_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER CURSO_ALUMNOS_TB_DELETE_TR
ON CURSO_ALUMNOS_TB
AFTER DELETE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (3, 'CURSO_ALUMNOS_TB', GETDATE(), 1, 1);
END;
GO

-------- TRIGGERS FIDE_CURSO_IMPARTIDO_TB --------
CREATE TRIGGER FIDE_CURSO_IMPARTIDO_TB_INSERT_TR
ON FIDE_CURSO_IMPARTIDO_TB
AFTER INSERT
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (1, 'FIDE_CURSO_IMPARTIDO_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_CURSO_IMPARTIDO_TB_UPDATE_TR
ON FIDE_CURSO_IMPARTIDO_TB
AFTER UPDATE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (2, 'FIDE_CURSO_IMPARTIDO_TB', GETDATE(), 1, 1);
END;
GO

CREATE TRIGGER FIDE_CURSO_IMPARTIDO_TB_DELETE_TR
ON FIDE_CURSO_IMPARTIDO_TB
AFTER DELETE
AS
BEGIN
    INSERT INTO FIDE_AUDITORIA_TB (ID_ACCION_AUDITORIA, TABLA_AFECTADA, FECHA_HORA, ID_DOCENTE, ID_ESTADO)
    VALUES (3, 'FIDE_CURSO_IMPARTIDO_TB', GETDATE(), 1, 1);
END;
GO


